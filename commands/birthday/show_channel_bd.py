...
class ShowChannelBD(command_template.Command):
    ...
    async def command(self, message: discord.Message):
        ...
        if channel_bd is None:
            await self.send_message_check_forbidden(message, "No birthday channel is set on this server.")
        else:
            await self.send_message_check_forbidden(message, "The birthday channel for this server is {}.".format(
